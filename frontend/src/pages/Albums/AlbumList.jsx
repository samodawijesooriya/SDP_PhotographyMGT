    ...
    const fetchAlbums = async () => {
      try {
        ...
        for (const album of response.data) {
          try {            console.log(`Fetching thumbnail for album ${album.id} from: ${url}/api/drive/${album.id}/thumbnail`);
  ...
  return (
    <div className="albums-container">
      ...
      <div className="albums-grid">
        {albums.length > 0 ? (
          albums.map((album) => (            <Link to={`/gallery/${album.id}`} key={album.id} className="album-card">
              <div className="album-thumbnail">
                {albumThumbnails[album.id] ? (
                  <img 
                    ...
                    onError={(e) => {
                      console.error(`Failed to load thumbnail for album ${album.id}`);
